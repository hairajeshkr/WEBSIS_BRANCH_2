...
public partial class STUDENT_GradeAsgnStudent : ClsPageEvents, IPageInterFace
{
    ClsNEPGradeAssignStudent ObjCls = new ClsNEPGradeAssignStudent();
    TextBox TxtMark;
    HiddenField HdnId, HdnRwIndex, HdnClassId, HdnDivId, HdnStudentId;
    DropDownList DdlStudStatus, DdlGrades, DdlGrade;
    Label LblMaxMark;
    ...
    protected override void Page_Load(object sender, EventArgs e)
    {
        try
        {
            ...
            if (!IsPostBack)
            {
                ...
                DataTable DTGrdSystem = (ObjCls.FnGetDataSet("select nId Id,cNEPGradingName Name from TblNEPGradeMaster") as DataSet).Tables[0];
                DdlGradingSystem.DataSource = DTGrdSystem;
                DdlGradingSystem.DataValueField = "Id";
                DdlGradingSystem.DataTextField = "Name";
                DdlGradingSystem.DataBind();
                DdlGradingSystem.Items.Insert(0, new ListItem("--select--", "0"));
    ...
    public override void FnInitializeForm()
    {
        ...
        ObjCls = new ClsNEPGradeAssignStudent(ref iCmpId, ref iBrId, ref iFaId, ref iAcId);
    ...
    public void FnAssignProperty()
    {
        ...
        ObjCls.GradingSystemId = ObjCls.FnIsNumeric(DdlGradingSystem.SelectedValue);
    ...
    public override void FnCancel()
    {
        ...
        DdlGradingSystem.SelectedValue = "0";
        TabContainer1.ActiveTabIndex = 0;
    }
            ...
            switch (((Button)sender).CommandName.ToString().ToUpper())
            {
                ...
                case "FIND":
                    ...
                    for (int i = 0; i < Rwcount; i++)
                    {
                        ...
                        DdlGrade.SelectedValue = DT.Rows[i]["GradeId"].ToString();
    ...
    protected void GrdVwRecords_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        ...
        try
        {
            if (e.Row.RowType == DataControlRowType.DataRow)
            {
                ...
                DdlGrades = (DropDownList)e.Row.FindControl("DdlGrade");

                int GrdId = ObjCls.FnIsNumeric(DdlGradingSystem.SelectedValue);

                DataTable ClsTP = (ObjCls.FnGetDataSet("select TCD.nId ID,TCD.cNEPGrade Name FROM TblNEPGradeDetails TCD inner join TblNEPGradeMaster TGM on TGM.nId=TCD.nNEPGradingID where TCD.nNEPGradingID=" + GrdId) as DataSet).Tables[0];
                DdlGrades.DataSource = ClsTP;
                ...
                DdlStudStatus.Attributes.Add("onchange", "return FnUpdateStudGrade('" + ViewState["TOKENNO"].ToString() + "','" + ViewState["ID"].ToString() + "','" + FnGetRights().COMPANYID + "','" + FnGetRights().BRANCHID + "','" + FnGetRights().ACYEARID + "','" + FnGetRights().FAYEARID + "','" + FnGetRights().TTYPE + "','" + FnGetRights().USERID + "','" + DdlTeacher.ClientID + "','" + DdlGroup.ClientID + "','" + DdlPaper.ClientID + "','" + DdlExam.ClientID + "','" + DdlSubExam.ClientID + "','" + DdlGradingSystem.ClientID + "', '" + HdnClassId.ClientID + "','" + HdnDivId.ClientID + "','" + HdnStudentId.ClientID + "','" + CtrlExamDate.ClientID + "','" + DdlStudStatus.ClientID + "','" + DdlGrades.ClientID + "');");

                DdlGrades.Attributes.Add("onchange", "return FnUpdateStudGrade('" + ViewState["TOKENNO"].ToString() + "','" + ViewState["ID"].ToString() + "','" + FnGetRights().COMPANYID + "','" + FnGetRights().BRANCHID + "','" + FnGetRights().ACYEARID + "','" + FnGetRights().FAYEARID + "','" + FnGetRights().TTYPE + "','" + FnGetRights().USERID + "','" + DdlTeacher.ClientID + "','" + DdlGroup.ClientID + "','" + DdlPaper.ClientID + "','" + DdlExam.ClientID + "','" + DdlSubExam.ClientID + "','" + DdlGradingSystem.ClientID + "', '" + HdnClassId.ClientID + "','" + HdnDivId.ClientID + "','" + HdnStudentId.ClientID + "','" + CtrlExamDate.ClientID + "','" + DdlStudStatus.ClientID + "','" + DdlGrades.ClientID + "');");
