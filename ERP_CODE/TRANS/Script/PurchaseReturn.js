function ValidateMasterData()  
{
    ...
    var TxtOptId = document.getElementById('ContentPlaceHolder1_TabContainer1_TabPanel1_CtrlGrdOptRefNo_HdnId');
    var TxtOpt = document.getElementById('ContentPlaceHolder1_TabContainer1_TabPanel1_CtrlGrdOptRefNo_TxtCaption');
    ...
    if (!isEmpty(TxtRefNo, 'Please enter Reference No.', TxtRefNo) &&
        ...
        isChildData(TxtAccId, 'Please select Valid Suppllier', TxtAcc) &&
        isChildData(TxtOptId, 'Please select Valid Bill No.', TxtOpt))
    {
...
function FnCalculateGrandTotal()
{
    ...
    var dGrandTotal = roundNumber(dTotal + dRoundOff + dTotalTax, 2);
...
function ValidateGenerateData()
{
    ...
    var DdlSalesAccount = document.getElementById('ContentPlaceHolder1_TabContainer1_TabPanel2_DdlSalesAccount');

    if (!isEmpty(TxtTrDate, 'Please enter invoice Date.', TxtTrDate) && FnCheckFaYearTrDate(TxtTrDate) &&
         isListSelected(DdlSalesAccount, 'Please select valid sales type.', DdlSalesAccount, 0))
    {
       return true;
    }
    else
