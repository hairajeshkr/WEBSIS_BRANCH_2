...
using System.Data;
public class ClsNEPMarkAssignStudent : ClsCommonBaseMaster, IMasterCommands
{

    private int nStaffId = 0, nPaperId = 0, nStudGroupId = 0, nExamId = 0, nStudentId = 0, nClassId = 0, nDivisionId = 0, nSyllabusId = 0, nExamSubId=0,nMaxMark=0,nStudStatusId;
    private Decimal nMark = 0;
    private DateTime dExamDate;
    private Boolean nMarkUploaded;


    public ClsNEPMarkAssignStudent() : base("TblAcademicMarkAssign", "ID", 0, 0, 0, 0)
    {
    }
    public ClsNEPMarkAssignStudent(ref int PrmCompanyId, ref int PrmBranchId, ref int PrmFaId, ref int PrmAcId)
    ...
    public override object ManipulateData(string PrmDbFlag)
    {
        ...
        objFields.AddParameterFields("Prm_nStudStatusId", System.Data.DbType.Boolean, StudStatusId);
        objFields.AddParameterFields("Prm_nTokenId", System.Data.DbType.Double, TokenNo);


        if (PrmDbFlag.Equals("S"))
        {
            return objDBManager.GetRecord("ProNEPStudententMarkEntry", objFields) as DataSet;
        }
        else
        {
            ArrayList objRetArrList = new ArrayList(objDBManager.UpdateRecord("ProNEPStudententMarkEntry", objFields));
