...
function FnUpdateStudMark(PrmTokenNo, PrmId, PrmCompanyId, PrmBranchId, PrmAcId, PrmFaId, PrmTType, PrmUserId, PrmStaffIdCtrl, PrmGrpCtrl, PrmPaperCtrl, PrmExamCtrl, PrmSubExamCtrl, PrmTxtMarkCtrl, PrmClassId, PrmDivId, PrmStuId, PrmMaxMark, ExamDate, PrmStudStatus,  PrmWeightage,  PrmFinalMaxMark) {
    ...
    var Weightage = document.getElementById(PrmWeightage).value;
    var FinalMaxMark = document.getElementById(PrmFinalMaxMark).value;

    var nMark = document.getElementById(PrmTxtMarkCtrl).value;
    if ((document.getElementById(PrmStudStatus).value) > PrmMaxMark) {
        alert("mark is greater than maximum mark");
    }

    if (document.getElementById(PrmStudStatus).value == "2" || document.getElementById(PrmStudStatus).value == "3") {
        document.getElementById(PrmTxtMarkCtrl).disabled = true;
        document.getElementById(PrmTxtMarkCtrl).style.backgroundColor = 'red';
            }
    ...
    Web_GetStudentService.FnUpdateStudentMarkEntry(PrmCompanyId, PrmBranchId, PrmAcId, PrmFaId, PrmTType, PrmUserId, PrmTokenNo, PrmId, StaffId, StudGrpId, PaperId, ExamId, ExamSubId, ClassId, DivId, StudentId, nMark, MaxMark, StudStatus, Weightage, FinalMaxMark, ValStudentMarkUpdateSuccess, ValFailed);

}
          
function FnUpdateStudGrade(PrmTokenNo, PrmId, PrmCompanyId, PrmBranchId, PrmAcId, PrmFaId, PrmTType, PrmUserId, PrmStaffIdCtrl, PrmGrpCtrl, PrmPaperCtrl, PrmExamCtrl, PrmSubExamCtrl, PrmDdlGradingSystemCtrl, PrmClassId, PrmDivId, PrmStuId, ExamDate, PrmStudStatus, PrmGradeId) {
    var StaffId = document.getElementById(PrmStaffIdCtrl).value;
    var StudGrpId = document.getElementById(PrmGrpCtrl).value;
    var PaperId = document.getElementById(PrmPaperCtrl).value;
    var ExamId = document.getElementById(PrmExamCtrl).value;
    ...
    var nGradingSystem = document.getElementById(PrmDdlGradingSystemCtrl).value;
    var gradeId = document.getElementById(PrmGradeId).value;

    if (document.getElementById(PrmStudStatus).value == "2" || document.getElementById(PrmStudStatus).value == "3") {
        document.getElementById(PrmGradeId).disabled = true;
        document.getElementById(PrmGradeId).style.backgroundColor = 'lime';
    }

    else {
        document.getElementById(PrmGradeId).disabled = false;

    }
    Web_GetStudentService.FnUpdateStudGrade(PrmCompanyId, PrmBranchId, PrmAcId, PrmFaId, PrmTType, PrmUserId, PrmTokenNo, PrmId, StaffId, StudGrpId, PaperId, ExamId, ExamSubId, nGradingSystem, ClassId, DivId, StudentId, gradeId, StudStatus, ValStudentMarkUpdateSuccess, ValFailed);
